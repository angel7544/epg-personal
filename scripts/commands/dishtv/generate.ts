...
async function main() {
  ...
  logger.info('Loading DishTV channels...')
  const dishtvXmlPath = 'sites/dishtv.in/dishtv.in.channels.xml'
  const xml = await storage.load(dishtvXmlPath)
  const parsedChannels = EPGGrabber.parseChannelsXML(xml)
  
  const dishtvChannels = new Collection(parsedChannels).map(
    (channel: Record<string, string>) => new Channel(channel as any)
  )

  logger.info(`Found ${dishtvChannels.count()} channels. Aligning IDs...`)

  const alignedChannels = new Collection<Channel>()
  const m3uLines: string[] = ['#EXTM3U']

  dishtvChannels.forEach((channel: Channel) => {
    let xmltvId = channel.xmltv_id

    // Clean up suffixes
    if (xmltvId) {
        xmltvId = xmltvId.replace(/@SD$/i, '')
                         .replace(/@HD$/i, '')
                         .replace(/@India$/i, '')
                         .replace(/@Global$/i, '')
                         .replace(/@SouthAsia$/i, '')
    ...
    if (isIndian) {
      ...
      // Add to M3U
      const streamUrl = `http://localhost:5000/live/dishtv/${channel.site_id}.m3u8`
      m3uLines.push(`#EXTINF:-1 tvg-id="${xmltvId}" tvg-logo="${channel.logo || ''}" group-title="DishTV",${channel.name}`)
  ...
  // Save dishtv.channels.xml
  const channelsXml = generateChannelsXML(alignedChannels)
  await storage.save('dishtv.channels.xml', channelsXml)
  logger.info('Saved dishtv.channels.xml')

  // Save dishtv.m3u
  await storage.save('dishtv.m3u', m3uLines.join('\n'))
  logger.info('Saved dishtv.m3u')
