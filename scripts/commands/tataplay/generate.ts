import { Storage } from '@freearhey/storage-js'
import { Collection, Logger } from '@freearhey/core'
import { EPGGrabber } from 'epg-grabber'
import { generateChannelsXML } from '../../core'
import { Channel } from '../../models'
import { loadData, data } from '../../api'

async function main() {
  const logger = new Logger()
  const storage = new Storage()

  logger.info('Loading iptv-org database...')
  await loadData()

  logger.info('Loading Tata Play channels...')
  const tataplayXmlPath = 'sites/tataplay.com/tataplay.com.channels.xml'
  const xml = await storage.load(tataplayXmlPath)
  const parsedChannels = EPGGrabber.parseChannelsXML(xml)
  
  const tataplayChannels = new Collection(parsedChannels).map(
    (channel: any) => new Channel(channel)
  )

  logger.info(`Found ${tataplayChannels.count()} channels. Aligning IDs...`)

  const alignedChannels = new Collection<Channel>()
  const m3uLines: string[] = ['#EXTM3U']

  tataplayChannels.forEach((channel: Channel) => {
    let xmltvId = channel.xmltv_id || ''

    // Clean up Tata Play specific suffixes (@SD, @HD, etc.)
    if (xmltvId.includes('@')) {
      xmltvId = xmltvId.split('@')[0]
    }

    // Attempt to find official ID if missing or invalid
    if (!xmltvId) {
      const match = Object.values(data.channelsKeyById.data()).find((c: any) => 
        c.name.toLowerCase() === channel.name.toLowerCase() ||
        c.name.toLowerCase().replace(/\s+/g, '') === channel.name.toLowerCase().replace(/\s+/g, '')
      )

      if (match) {
        xmltvId = match.id
      } else {
        xmltvId = `${channel.name.replace(/\s+/g, '')}.in`
      }
    }

    // Only include Indian channels
    const officialChannel = data.channelsKeyById.get(xmltvId)
    const isIndian = xmltvId.endsWith('.in') || (officialChannel && officialChannel.country === 'IN')

    if (isIndian) {
      channel.xmltv_id = xmltvId
      alignedChannels.add(channel)

      // Add to M3U
      const streamUrl = `http://localhost:5000/live/tataplay/${channel.site_id}.m3u8` // Default placeholder
      m3uLines.push(`#EXTINF:-1 tvg-id="${xmltvId}" tvg-logo="${channel.logo || ''}" group-title="TataPlay",${channel.name}`)
      m3uLines.push(streamUrl)
    }
  })

  logger.info(`Matched ${alignedChannels.count()} Indian channels.`)

  // Save tataplay.channels.xml
  const channelsXml = generateChannelsXML(alignedChannels)
  await storage.save('tataplay.channels.xml', channelsXml)
  logger.info('Saved tataplay.channels.xml')

  // Save tataplay.m3u
  await storage.save('tataplay.m3u', m3uLines.join('\n'))
  logger.info('Saved tataplay.m3u')

  logger.success('Generation complete!')
  logger.info('Now run: npm run grab --- --channels=tataplay.channels.xml --output=tataplay.xml')
}

main().catch(console.error)
